class DeliveryTrackersController < ApplicationController
  ...
  def create
    ...
    the_delivery_tracker.user_id = session.fetch(:user_id)
    the_delivery_tracker.status ="waiting_on"

    if the_delivery_tracker.valid?
      the_delivery_tracker.save
      redirect_to("/delivery_trackers", {:notice => "Added to list."})
    else
  ...
  def update
    ...
    if the_delivery_tracker.valid?
      the_delivery_tracker.save
      redirect_to("/delivery_trackers", { :notice => "Delivery tracker updated successfully."} )
    else
      redirect_to("/delivery_trackers", { :alert => the_delivery_tracker.errors.full_messages.to_sentence })
    end
  end
